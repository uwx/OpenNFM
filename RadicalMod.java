...
public class RadicalMod {
	...
	public RadicalMod(String string, int i, int i_0, final int i_1, final boolean bool, final boolean bool_2) {
		...
		try {
			...
			if (module.isLoaded()) {
				...
				// sClip.rollBackTrig = moduleslayer.oln -
				// moduleslayer.rollBackTrig;
				if (bool_2)
					filename = (new StringBuilder()).append("Length: ")
							.append(getTimer(sClip.stream.available() / 44100)).toString();
				loaded = 2;
			}
	...
	public RadicalMod(final String string, final Applet applet) {
		playing = false;
		loaded = 0;
		rvol = 0;
		imod = "";
	...
	public void loadMod(final int i, final int j, final int k, final boolean b1, final boolean b2) {
		// stracks[16].loadMod(400, 7600, 125, sunny, macn);
		loadimod(i, j, k, b1, b2);
	}

	public void loadimod(final int i, final int j /* ignored */, final int i_7, final boolean bool, final boolean b2) {
		if (loaded == 1) {
			final int i2 = 44000;
			int i_6 = 160;
			if (bool)
			...
			try {
				...
				if (module.isLoaded()) {
					...
					if (name.trim().equals(""))
						name = "Untitled";
					final ModuleSlayer moduleslayer = ModuleLoader.prepareSlayer(module, i2, i_6, i_7);
					final byte is[] = moduleslayer.turnbytesNorm(bool);
					if (bool)
						rvol = moduleslayer.olav;
		...
		if (loaded == 1) {
			...
			try {
				...
				if (module.isLoaded()) {
					...
					if (name.trim().equals(""))
						name = "Untitled";
					final ModuleSlayer moduleslayer = ModuleLoader.prepareSlayer(module, refreshRate, i_6, i_7);
					final byte is[] = moduleslayer.turnbytesNorm(bool);
					if (bool)
						rvol = moduleslayer.olav;
					sClip = new SuperClip(is, moduleslayer.oln, 22000);
