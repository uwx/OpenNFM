...
public class CustomFileDemo {
					...
					public void run() {
						...
						try {
							final URL url = new URL((new StringBuilder())
									.append("http://multiplayer.needformadness.com/cars/lists/").append(s)
									.append(".txt?reqlo=").append((int) (Math.random() * 1000D)).append("").toString());
						...
						for (int i2 = 0; i2 < args1.length; i2++)
							System.out.println(args1[i2]);
						System.out.println("----------------------");
						final int k = args1.length;
						for (int l = 0; l < k;) {
							lblNewLabel.setText("Downloading car " + args1[l]);
							frame.pack();
							frame.setSize(frame.getWidth(), 128);
							final String s2 = args1[l];
							try {
								String s3 = (new StringBuilder()).append("http://multiplayer.needformadness.com/cars/")
										.append(s2).append(".radq?reqlo=").append((int) (Math.random() * 1000D))
										.toString();
								s3 = s3.replace(' ', '_');
								final URL url1 = new URL(s3);
								final int i1 = url1.openConnection().getContentLength();
								final DataInputStream datainputstream1 = new DataInputStream(
										new BufferedInputStream(url1.openStream()));
								final byte abyte0[] = new byte[i1];
								...
								if (abyte0[0] == 80 && abyte0[1] == 75 && abyte0[2] == 3)
									...
									final byte abyte1[] = new byte[i1 - 40];
								...
								s5 = (new StringBuilder()).append(s5).append("\n\n").toString();
								File file = new File(
										(new StringBuilder()).append("C:\\NFM Cars\\").append(s).toString());
								if (!file.exists())
									file.mkdirs();
								file = new File((new StringBuilder()).append("C:\\NFM Cars\\").append(s).append("\\")
										.append(s2).append(".rad").toString());
			...
			public void actionPerformed(final ActionEvent e) {
				...
				new Thread(new Runnable() {
					...
					public void run() {
						...
						try {
							final URL url = new URL((new StringBuilder())
									.append("http://multiplayer.needformadness.com/tracks/lists/").append(s)
									.append(".txt?reqlo=").append((int) (Math.random() * 1000D)).append("").toString());
							final DataInputStream datainputstream = new DataInputStream(
									new BufferedInputStream(url.openStream()));
							String s1 = datainputstream.readLine().trim();
						...
						for (int l = 0; l < k;) {
							...
							try {
								String s3 = (new StringBuilder())
										.append("http://multiplayer.needformadness.com/tracks/").append(s2)
										.append(".radq?reqlo=").append((int) (Math.random() * 1000D)).toString();
								s3 = s3.replace(' ', '_');
								final URL url1 = new URL(s3);
								final int i1 = url1.openConnection().getContentLength();
								final DataInputStream datainputstream1 = new DataInputStream(
								...
								for (int k2 = s4.indexOf("\n", 0); k2 != -1 && j2 < s4.length();) {
									...
									if (s6.startsWith("soundtrack(")) {
										final String s23 = getstr("soundtrack", s6, 0);
										...
										try {
											String s8 = (new StringBuilder())
													...
													.append(s23).append(".zip").toString();
											s8 = s8.replace(' ', '_');
											final URL url3 = new URL(s8);
											final int j15 = url3.openConnection().getContentLength();
											File file6 = new File("C:\\NFM Tracks\\music\\");
											if (!file6.exists())
												file6.mkdirs();
											file6 = new File(
													(new StringBuilder()).append("").append("C:\\NFM Tracks\\music\\")
															.append(s23).append(".zip").toString());
											if (file6.exists())
												if (file6.length() == j15)
													l3 = 1;
											...
											if (l3 == 0) {
												final DataInputStream datainputstream3 = new DataInputStream(
														url3.openStream());
												final byte abyte1[] = new byte[j15];
								...
								s5 = (new StringBuilder()).append(s5).append("\n\n").toString();
								File file = new File(
										(new StringBuilder()).append("C:\\NFM Tracks\\").append(s).toString());
								if (!file.exists())
									file.mkdirs();
								file = new File((new StringBuilder()).append("C:\\NFM Tracks\\").append(s).append("\\")
										.append(s2).append(".txt").toString());
								BufferedWriter bufferedwriter = new BufferedWriter(new FileWriter(file));
								bufferedwriter.write(s5);
								bufferedwriter.close();
								bufferedwriter = null;
	...
	public static void main(final String args[]) throws IOException {
		...
		/*
		 * String args3[] = { "AudiR8King" }; dltracks(args3);
		 */
	}

	public static void dlcars(final String[] args3) {
		final int i = args3.length;
		for (int j = 0; j < i; j++) {
			System.out.println("Downloading cars from " + args3[j]);
			final String s = args3[j];
			String args1[];
			try {
				final URL url = new URL(
						(new StringBuilder()).append("http://multiplayer.needformadness.com/cars/lists/").append(s)
								.append(".txt?reqlo=").append((int) (Math.random() * 1000D)).append("").toString());
			...
			final String args4[] = args1;
			System.out.println("---- LIST OF CARS ----");
			for (int i2 = 0; i2 < args1.length; i2++)
				System.out.println(args1[i2]);
			System.out.println("----------------------");
			final int k = args4.length;
			for (int l = 0; l < k;) {
				System.out.println("Downloading car " + args4[l]);
				final String s2 = args4[l];
				try {
					String s3 = (new StringBuilder()).append("http://multiplayer.needformadness.com/cars/").append(s2)
							.append(".radq?reqlo=").append((int) (Math.random() * 1000D)).toString();
					s3 = s3.replace(' ', '_');
					final URL url1 = new URL(s3);
					final int i1 = url1.openConnection().getContentLength();
					...
					if (!file.exists())
						file.mkdirs();
					file = new File((new StringBuilder()).append("C:\\NFM Cars\\").append(s).append("\\").append(s2)
							.append(".rad").toString());
					BufferedWriter bufferedwriter = new BufferedWriter(new FileWriter(file));
					bufferedwriter.write(s5);
					bufferedwriter.close();
	...
	public static void dltracks(final String[] args3) {
		// dl tracks
		final int i = args3.length;
		for (int j = 0; j < i; j++) {
			...
			try {
				final URL url = new URL(
						(new StringBuilder()).append("http://multiplayer.needformadness.com/tracks/lists/").append(s)
								.append(".txt?reqlo=").append((int) (Math.random() * 1000D)).append("").toString());
				final DataInputStream datainputstream = new DataInputStream(new BufferedInputStream(url.openStream()));
				String s1 = datainputstream.readLine().trim();
				s1 = s1.substring(s1.indexOf("(") + 1, s1.length() - 1);
			...
			for (int l = 0; l < k;) {
				...
				final String s2 = args4[l];
				try {
					String s3 = (new StringBuilder()).append("http://multiplayer.needformadness.com/tracks/").append(s2)
							.append(".radq?reqlo=").append((int) (Math.random() * 1000D)).toString();
					s3 = s3.replace(' ', '_');
					final URL url1 = new URL(s3);
					final int i1 = url1.openConnection().getContentLength();
					...
					if (!file.exists())
						file.mkdirs();
					file = new File((new StringBuilder()).append("C:\\NFM Tracks\\").append(s).append("\\").append(s2)
							.append(".txt").toString());
					BufferedWriter bufferedwriter = new BufferedWriter(new FileWriter(file));
					bufferedwriter.write(s5);
					bufferedwriter.close();
	...
	static int getint(final String s, final String s1, final int i) {
		int k = 0;
		String s3 = "";
	...
	static String getstr(final String s, final String s1, final int i) {
		int k = 0;
		String s3 = "";
		for (int j = s.length() + 1; j < s1.length(); j++) {
			final String s2 = (new StringBuilder()).append("").append(s1.charAt(j)).toString();
	...
	static void dlmusic(final String s23) {
		System.out.println("Downloading sound track " + s23);
		int l3 = 0;
		try {
			String s8 = (new StringBuilder()).append("http://multiplayer.needformadness.com/tracks/music/").append(s23)
					.append(".zip").toString();
			s8 = s8.replace(' ', '_');
			final URL url3 = new URL(s8);
			final int j15 = url3.openConnection().getContentLength();
			...
			if (!file6.exists())
				file6.mkdirs();
			file6 = new File((new StringBuilder()).append("").append("C:\\NFM Tracks\\music\\").append(s23)
					.append(".zip").toString());
			if (file6.exists())
				if (file6.length() == j15)
					l3 = 1;
			...
			if (l3 == 0) {
				final DataInputStream datainputstream3 = new DataInputStream(url3.openStream());
				final byte abyte1[] = new byte[j15];
				datainputstream3.readFully(abyte1);
				datainputstream3.close();
				FileOutputStream fileoutputstream1 = new FileOutputStream(file6);
			...
			System.out.println("Could not download sound track " + s23);
			exception13.printStackTrace();
		}
	}
