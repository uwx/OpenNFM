...
public class Control {
	...
	public void preform(final Madness madness, final ContO conto, final CheckPoints checkpoints,
			final Trackers trackers, final int ncars) {
		left = false;
		right = false;
		up = false;
		down = false;
					...
					if (attack == 0) {
						...
						do
							if (i4 != madness.im && checkpoints.clear[i4] != -1) {
								...
								int i8;
								for (i8 = (int) (90 + c4
										+ Math.atan((double) (checkpoints.opz[i4] - conto.z)
			...
			if (flag) {
				...
				if (attack == 0) {
					...
					if (turncnt > randtcnt) {
						if (!gowait) {
							...
							if (wtx - conto.x >= 0)
								c2 = '\264';
							pan = (int) (90 + c2 + Math.atan((double) (wtz - conto.z) / (double) (wtx - conto.x))
									/ 0.017453292519943295D);
						}
						turncnt = 0;
					...
					up = true;
					char c = '\0';
					final int l1 = (int) ((pys(conto.x, checkpoints.opx[acr], conto.z, checkpoints.opz[acr]) / 2.0F)
							* aim);
					final int l3 = (int) (checkpoints.opx[acr] - l1 * m.sin(checkpoints.omxz[acr]));
					final int k5 = (int) (checkpoints.opz[acr] + l1 * m.cos(checkpoints.omxz[acr]));
					if (l3 - conto.x >= 0)
						c = '\264';
					pan = (int) (90 + c
					...
					if (checkpoints.stage == 15 && exitattack && !bulistc && madness.missedcp != 0)
						attack = 0;
					if (checkpoints.stage == 16 && madness.cn == 13
				...
				if (hold == 0)
					if (Math.abs(j - pan) < 180) {
						...
						if (Math.abs(j - pan) < 310 && madness.speed > madness.swits[madness.cn][0] && turntyp != 0) {
							if (turntyp == 1)
								down = true;
							if (turntyp == 2)
								handb = true;
				...
				if (trickfase == 0) {
					final int k = (int) (((madness.scy[0] + madness.scy[1] + madness.scy[2] + madness.scy[3])
							* (conto.y - 300)) / 4000F);
					int i2 = 3;
					if (checkpoints.stage == 15)
						i2 = 10;
					if (k > 7 && (m.random() > trickprf / i2 || stuntf == 4 || stuntf == 3 || stuntf == 5 || stuntf == 6
							|| checkpoints.stage == 16)) {
						oxy = madness.pxy;
						ozy = madness.pzy;
						flycnt = 0;
