...
public class Medium /////// This entire class was re-edited by Chaotic for full
					/////// NFMM Graphics (excluding sparks and extra dust)
{
	int nmt = 0;
	int[] mrd = null;
	int[] nmv = null;
	...
	public void newpolys(final int i, final int j, final int k, final int l, final Trackers trackers, final int i1) {
		final Random random1 = new Random((i1 + cgrnd[0] + cgrnd[1] + cgrnd[2]) * 1671);
		nrw = j / 1200 + 9;
		ncl = l / 1200 + 9;
		sgpx = i - 4800;
		sgpz = k - 4800;
		...
		for (int i2 = 0; i2 < nrw * ncl; i2++) {
			final float f = (float) (0.29999999999999999D + 1.6000000000000001D * random1.nextDouble());
			ogpx[i2][0] = 0;
			...
			for (int k2 = 0; k2 < 8; k2++) {
				...
				final int j3 = (int) Math.sqrt((int) (ogpx[i2][k2] * ogpx[i2][k2] * pvr[i2][k2] * pvr[i2][k2]
	...
	public void groundpolys(final Graphics2D graphics2d) {
		...
		if (l < k)
			l = k;
		final int ai[][] = new int[j - i][l - k];
		for (int i1 = i; i1 < j; i1++)
			for (int k1 = k; k1 < l; k1++) {
				ai[i1 - i][k1 - k] = 0;
				final int i2 = i1 + k1 * nrw;
				if (resdown >= 2 && i2 % 2 != 0)
					continue;
				final int k2 = cx + (int) ((cgpx[i2] - x - cx) * cos(xz) - (cgpz[i2] - z - cz) * sin(xz));
				final int l2 = cz + (int) ((cgpx[i2] - x - cx) * sin(xz) + (cgpz[i2] - z - cz) * cos(xz));
				final int i3 = cz + (int) ((250 - y - cy) * sin(zy) + (l2 - cz) * cos(zy));
				if (xs(k2 + pmx[i2], i3) <= 0 || xs(k2 - pmx[i2], i3) >= w || i3 <= -pmx[i2] || i3 >= fade[2])
					continue;
				ai[i1 - i][k1 - k] = i3;
				final int ai4[] = new int[8];
				final int ai6[] = new int[8];
				final int ai8[] = new int[8];
				...
				final int ai9[] = new int[8];
				final int ai10[] = new int[8];
				int k4 = 0;
				int i5 = 0;
				int j5 = 0;
				int i6 = 0;
	...
	public void newclouds(int i, int j, int k, int l) {
		...
		for (int i1 = 0; i1 < noc; i1++) {
			...
			final float f = (float) (0.25D + Math.random() * 1.25D);
			...
			for (int l1 = 0; l1 < 12; l1++) {
				...
				final int j3 = (int) Math.sqrt(clax[i1][2][l1] * clax[i1][2][l1] + claz[i1][2][l1] * claz[i1][2][l1]);
				if (j3 > cmx[i1])
					cmx[i1] = j3;
			}
			...
			for (int i2 = 0; i2 < 6; i2++) {
				...
				for (int k3 = 0; k3 < 3; k3++) {
					final float f3 = clds[k3] * 1.05F - clds[k3];
	...
	public void drawclouds(final Graphics2D graphics2d) {
		for (int i = 0; i < noc; i++) {
			final int j = cx + (int) ((clx[i] - x / 20 - cx) * cos(xz) - (clz[i] - z / 20 - cz) * sin(xz));
			final int k = cz + (int) ((clx[i] - x / 20 - cx) * sin(xz) + (clz[i] - z / 20 - cz) * cos(xz));
			final int l = cz + (int) ((cldd[4] - y / 20 - cy) * sin(zy) + (k - cz) * cos(zy));
			final int i1 = xs(j + cmx[i], l);
			final int j1 = xs(j - cmx[i], l);
			if (i1 <= 0 || j1 >= w || l <= -cmx[i] || i1 - j1 <= 20)
				continue;
			final int ai[][] = new int[3][12];
			final int ai1[][] = new int[3][12];
			final int ai2[][] = new int[3][12];
			final int ai3[] = new int[12];
			final int ai4[] = new int[12];
			int k1 = 0;
			int l1 = 0;
			int i2 = 0;
			int j2 = 0;
			...
			for (int k3 = 0; k3 < 12; k3 += 2) {
				...
				k2 /= 6;
				i3 /= 6;
				final int i5 = (int) Math.sqrt((cy - k2) * (cy - k2) + (cx - l2) * (cx - l2) + i3 * i3);
				if (i5 >= fade[7])
					continue;
			...
			for (int l3 = 0; l3 < 12; l3 += 2) {
				...
				k2 /= 6;
				i3 /= 6;
				final int k5 = (int) Math.sqrt((cy - k2) * (cy - k2) + (cx - l2) * (cx - l2) + i3 * i3);
				if (k5 >= fade[7])
					continue;
			...
			k2 /= 12;
			i3 /= 12;
			final int j4 = (int) Math.sqrt((cy - k2) * (cy - k2) + (cx - l2) * (cx - l2) + i3 * i3);
			if (j4 >= fade[7])
				continue;
			int l5 = clds[0];
			int i7 = clds[1];
					...
					if (bool_201_) {
						...
						if (f > 0.0F && f < 10.0F) {
							...
							final int i_211_ = (int) ((mtc[i_185_][i_202_][2] + cgrnd[2] + csky[2] * f + cfade[2] * f)
									/ (2.0F + f * 2.0F));
							rd.setColor(new Color(i_209_, i_210_, i_211_)); // Mountain
																			// Color,
																			// should
																			// make
																			// this
	...
	public void newstars() {
		...
		nst = 0;
		if (lightson) {
			final Random random1 = new Random((long) (Math.random() * 100000D));
			nst = 40;
			stx = new int[nst];
			stz = new int[nst];
			stc = new int[nst][2][3];
	...
	public void drawstars(final Graphics2D graphics2d) {
		for (int i = 0; i < nst; i++) {
			...
			final int k = cz + (int) (stx[i] * sin(xz) + stz[i] * cos(xz));
			int l = cy + (int) (-200F * cos(zy) - k * sin(zy));
			final int i1 = cz + (int) (-200F * sin(zy) + k * cos(zy));
			j = xs(j, i1);
			l = ys(l, i1);
	...
	public void d(final Graphics2D graphics2d) {
		...
		final int k3 = cz + (int) ((skyline - 700 - cy) * sin(zy) + (7000 - cz) * cos(zy));
		j3 = ys(j3, k3);
		int l3 = ih;
	...
	public void around(final ContO conto, final boolean flag) {
		if (flex != 0)
			flex = 0;
		if (!flag) {
			if (!vert)
	...
	public void setsky(final int i, final int j, final int k) {
		osky[0] = i;
		osky[1] = j;
		osky[2] = k;
		for (int l = 0; l < 3; l++) {
	...
	public void setcloads(final int i, final int j, final int k, int l, int i1) {
		if (l < 0)
			l = 0;
		if (l > 10)
			l = 10;
	...
	public void setgrnd(final int i, final int j, final int k) {
		ogrnd[0] = i;
		ogrnd[1] = j;
		ogrnd[2] = k;
		for (int l = 0; l < 3; l++) {
	...
	public void setpolys(final int i, final int j, final int k) {
		cpol[0] = (int) (i + i * (snap[0] / 100F));
		if (cpol[0] > 255)
			cpol[0] = 255;
		if (cpol[0] < 0)
	...
	public void follow(final ContO conto, int i, final int j) {
		zy = 10;
	...
	public void eagle(final ContO conto, final int i) {
		zy = 25;
		xz = -i;
		x = (conto.x - cx) + (int) ((-(conto.z - 1000 - conto.z)) * sin(i));
		z = (conto.z - cz) + (int) ((conto.z - 1000 - conto.z) * cos(i));
		y = conto.y - 850 - cy;
		if (trns != 1)
			trns = 1;
	}

	public void gopro(final Madness madness, final ContO conto, final int i) {
		// zy = 10 + (int)madness.af2[0];
		xz = -i;
		x = (conto.x - cx) + (int) ((-(conto.z - 1 - conto.z)) * sin(i));
		z = (conto.z - cz) + (int) ((conto.z - 1 - conto.z) * cos(i));
	...
	public void getaround(final ContO conto) {
		...
		if (i < 1000)
			i = 1000;
		final int j = conto.y - adv;
		final int k = conto.x + (int) ((conto.x - i - conto.x) * cos(vxz));
		final int l = conto.z + (int) ((conto.x - i - conto.x) * sin(vxz));
		int i1 = 0;
		...
		if (conto.y - l1 - cy < 0)
			c = '\uFF4C';
		final int i2 = (int) Math
				.sqrt(((conto.z - z) + cz) * ((conto.z - z) + cz) + (conto.x - x - cx) * (conto.x - x - cx));
		int j2 = 25;
		if (i2 != 0)
			j2 = (int) (90 + c - Math.atan((double) i2 / (double) (conto.y - l1 - cy)) / 0.017453292519943295D);
		for (; j1 < 0; j1 += 360) {
		}
		for (; j1 > 360; j1 -= 360) {
		}
	...
	public void transaround(final ContO conto, final ContO conto1, final int i) {
		if (flex != 0)
			flex = 0;
		final int j = (conto.x * (20 - i) + conto1.x * i) / 20;
		final int k = (conto.y * (20 - i) + conto1.y * i) / 20;
		final int l = (conto.z * (20 - i) + conto1.z * i) / 20;
		if (!vert)
			adv += 2;
		else
			adv -= 2;
		...
		if (k - j1 - cy < 0)
			c = '\uFF4C';
		final int k1 = (int) Math.sqrt(((l - z) + cz) * ((l - z) + cz) + (j - x - cx) * (j - x - cx));
		final int l1 = (int) (90 + c - Math.atan((double) k1 / (double) (k - j1 - cy)) / 0.017453292519943295D);
		xz = -vxz + 90;
		zy += (l1 - zy) / 10;
		if (trns != 5)
			trns = 5;
