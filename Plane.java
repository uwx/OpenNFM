...
public class Plane {
	...
	public Plane(final Medium medium, final Trackers trackers, final int ai[], final int ai1[], final int ai2[],
			final int i, final int ai3[], final boolean flag, final int j, final int k, final int l, final int i1,
			final int j1, final int k1, final int l1, final boolean flag1, final int i2, final boolean random) {
		this.random = random;
		c = new int[3];
		oc = new int[3];
		hsb = new float[3];
		...
		final int k2 = Math.abs(ox[2] - ox[1]);
		final int l2 = Math.abs(oy[2] - oy[1]);
		final int i3 = Math.abs(oz[2] - oz[1]);
		if (l2 <= k2 && l2 <= i3)
			typ = 2;
		if (k2 <= l2 && k2 <= i3)
			typ = 1;
	...
	public Plane(final Medium medium, final Trackers trackers, final int ai[], final int ai1[], final int ai2[],
			final int i, final int ai3[], final boolean flag, final int j, final int k, final int l, final int i1,
			final int j1, final int k1, final int l1, final boolean flag1, final int i2) {
		random = false;
		c = new int[3];
		oc = new int[3];
		hsb = new float[3];
	...
	public void d(final Graphics2D rd, final int i, final int j, final int k, final int l, final int i1, final int j1,
			final int k1, final int l1, boolean flag, final int i2) {
		if (master != 0)
			if (av > 1500)
				n = 8;
			else
		...
		if (embos == 0) {
			...
			if (embos > 4 && embos <= 7)
				f = 1.0F + m.random() / 4F;
			if (embos > 7 && embos <= 9) /////// /// explode outward a little +
											/////// white flame a bit bigger
											/////// than previous + polys start
											/////// to darken
			{
				f = 1.0F + m.random() / 3F;
			...
			if (embos > 9 && embos <= 10) /////// /// explode outward a little +
											/////// white flame a bit bigger
											/////// than previous + maor
											/////// darkening
			{
			...
			if (embos > 10 && embos <= 12) /////// /// explode outward a little
											/////// + white flame a bit bigger
											/////// than previous + more darkkkk
											/////// liek coffee beans
			{
			...
			if (embos >= 16) ///// flames as well
			{
				...
				while (Math.abs(ai1[0] - ai1[1]) > 100)
					if (ai1[1] > ai1[0])
						...
						ai1[1] += 30;
				final int i7 = (int) (Math.abs(ai[0] - ai[1]) / 3 * (0.5D - m.random()));
				final int l7 = (int) (Math.abs(ai1[0] - ai1[1]) / 3 * (0.5D - m.random()));
				ai[2] = (ai[0] + ai[1]) / 2 + i7;
		...
		if (chip != 0) {
			...
			if (bfase != -7) {
				if (l5 == 0)
					/*
					 * if (glass) { final int transp[] = new int[3]; transp[0] =
					 * Color.getHSBColor(c[0], c[1], c[2]).getRed(); transp[1] =
					 * Color.getHSBColor(c[0], c[1], c[2]).getGreen(); transp[2]
					 * = Color.getHSBColor(c[0], c[1], c[2]).getBlue();
					 * rd.setColor(new Color(transp[0], transp[1], transp[2],
					 * 100).darker()); } else {
					 */
				...
				/*
				 * if (glass) { final int transp[] = new int[3]; transp[0] =
				 * Color.getHSBColor(hsb[0], hsb[1], hsb[2]).getRed(); transp[1]
				 * = Color.getHSBColor(hsb[0], hsb[1], hsb[2]).getGreen();
				 * transp[2] = Color.getHSBColor(hsb[0], hsb[1],
				 * hsb[2]).getBlue(); rd.setColor(new Color(transp[0],
				 * transp[1], transp[2], 100)); } else {
				 */
		...
		rot(ai, ai1, m.cx, m.cz, m.xz, n);
		boolean flag1 = false;
		final int ai7[] = new int[n];
		final int ai9[] = new int[n];
		int l4 = 500;
		...
		rot(ai2, ai1, m.cy, m.cz, m.zy, n);
		boolean flag2 = true;
		final int ai14[] = new int[n];
		final int ai15[] = new int[n];
		int j8 = 0;
		int l8 = 0;
		int j9 = 0;
		int l9 = 0;
		...
		if (flag2) {
			...
			final int l17 = (j14 + k15) / 2;
			final int j18 = (k16 + l16) / 2;
			final int l18 = (i17 + j17) / 2;
			av = (int) Math
		...
		if (flag2) {
			...
			if (glass)
				rd.setColor(new Color(l11, j13, k14, 150)); // red, green, blue,
															// alpha
			else if (random)
			...
			if (!flag) {
				if (flx == 0) {
					...
					/*
					 * if (glass) { final int transp[] = new int[3]; transp[0] =
					 * new Color(l11, j13, k14).getRed(); transp[1] = new
					 * Color(l11, j13, k14).getGreen(); transp[2] = new
					 * Color(l11, j13, k14).getBlue(); rd.setColor(new
					 * Color(transp[0], transp[1], transp[2], 100)); } else {
					 */
	...
	public void s(final Graphics2D rd, final int i, final int j, final int k, final int l, final int i1, final int j1,
			final int k1) {
		final int ai[] = new int[n];
		final int ai1[] = new int[n];
		final int ai2[] = new int[n];
		...
		if (k1 == 0) {
			...
			for (int l6 = t.nt - 1; l6 >= 0; l6--) {
				...
				for (int j8 = 0; j8 < n; j8++) {
					ai2[j8] = t.y[l6] - m.y;
					if (t.zy[l6] != 0)
						ai2[j8] += ((ai1[j8] - (t.z[l6] - m.z - t.radz[l6])) * m.sin(t.zy[l6])) / m.sin(90 - t.zy[l6])
								- (t.radz[l6] * m.sin(t.zy[l6])) / m.sin(90 - t.zy[l6]);
					if (t.xy[l6] != 0)
						ai2[j8] += ((ai[j8] - (t.x[l6] - m.x - t.radx[l6])) * m.sin(t.xy[l6])) / m.sin(90 - t.xy[l6])
								- (t.radx[l6] * m.sin(t.xy[l6])) / m.sin(90 - t.xy[l6]);
				}
