...
public class ContO {
	...
	public void pdust(final int i, final Graphics2D rd, final int j) {
		if (j * dov[i] > 0) {
			int k;
			if (fulls[i])
				k = stg[i] * stg[i];
			...
			if (sy[i] > 250)
				sy[i] = 250;
			final int l1 = m.cx + (int) ((sx[i] - m.x - m.cx) * m.cos(m.xz) - (sz[i] - m.z - m.cz) * m.sin(m.xz));
			int i2 = m.cz + (int) ((sx[i] - m.x - m.cx) * m.sin(m.xz) + (sz[i] - m.z - m.cz) * m.cos(m.xz));
			final int j2 = m.cy + (int) ((sy[i] - m.y - m.cy) * m.cos(m.zy) - (i2 - m.cz) * m.sin(m.zy));
			i2 = m.cz + (int) ((sy[i] - m.y - m.cy) * m.sin(m.zy) + (i2 - m.cz) * m.cos(m.zy));
			final int k2 = (int) Math.sqrt((m.cy - j2) * (m.cy - j2) + (m.cx - l1) * (m.cx - l1) + i2 * i2);
			int l2 = 0;
			do
				if (k2 > m.fade[l2]) {
			...
			final int ai1[] = new int[8];
			final int i3 = stg[i] - 3;
			ai[0] = xs((int) (l1 - (18F + m.random() * 18F + i3 * 6) * smag[i]), i2);
			ai1[0] = ys((int) (j2 - (7.5D + m.random() * 7.5D + i3 * 2.5D) * smag[i]), i2);
			if (ai1[0] < 45 && m.flex != 0)
				m.flex = 0;
			ai[1] = xs((int) (l1 - (18F + m.random() * 18F + i3 * 6) * smag[i]), i2);
			ai1[1] = ys((int) (j2 + (7.5D + m.random() * 7.5D + i3 * 2.5D) * smag[i]), i2);
			ai[2] = xs((int) (l1 - (7.5D + m.random() * 7.5D + i3 * 2.5D) * smag[i]), i2);
			ai1[2] = ys((int) (j2 + (18F + m.random() * 18F + i3 * 6) * smag[i]), i2);
			ai[3] = xs((int) (l1 + (7.5D + m.random() * 7.5D + i3 * 2.5D) * smag[i]), i2);
			ai1[3] = ys((int) (j2 + (18F + m.random() * 18F + i3 * 6) * smag[i]), i2);
			ai[4] = xs((int) (l1 + (18F + m.random() * 18F + i3 * 6) * smag[i]), i2);
			ai1[4] = ys((int) (j2 + (7.5D + m.random() * 7.5D + i3 * 2.5D) * smag[i]), i2);
			ai[5] = xs((int) (l1 + (18F + m.random() * 18F + i3 * 6) * smag[i]), i2);
			ai1[5] = ys((int) (j2 - (7.5D + m.random() * 7.5D + i3 * 2.5D) * smag[i]), i2);
			ai[6] = xs((int) (l1 + (7.5D + m.random() * 7.5D + i3 * 2.5D) * smag[i]), i2);
			ai1[6] = ys((int) (j2 - (18F + m.random() * 18F + i3 * 6) * smag[i]), i2);
			ai[7] = xs((int) (l1 - (7.5D + m.random() * 7.5D + i3 * 2.5D) * smag[i]), i2);
	...
	public ContO(final byte abyte0[], final Medium medium, final Trackers trackers) {
		npl = 0;
		x = 0;
		y = 0;
		z = 0;
		...
		float f = 1.0F;
		float f1 = 1.0F;
		final int ai[] = new int[500]; // x points
		final int ai1[] = new int[500]; // y points
		final int ai2[] = new int[500]; // z points
		final int ai3[] = new int[3]; // colors
		boolean flag2 = false;
		boolean flag3 = false;
			...
			while ((s = datainputstream.readLine()) != null) {
				...
				if (flag) {
					...
					if (s1.startsWith("p")) {
						...
						final int j1 = (int) Math.sqrt(ai[i] * ai[i] + ai1[i] * ai1[i] + ai2[i] * ai2[i]);
						if (j1 > maxR)
							maxR = j1;
						i++;
					}
				...
				if (s1.startsWith("tracks")) {
					final int k1 = getvalue("tracks", s1, 0);
					txy = new int[k1];
					tzy = new int[k1];
					tc = new int[k1][3];
					tradx = new int[k1];
	...
	public ContO(final ContO conto, final int i, final int j, final int k, final int l) {
		...
		if (conto.tnt != 0)
			for (int k1 = 0; k1 < conto.tnt; k1++) {
				...
				if (i2 == 180)
					i2 = 0;
				t.radx[t.nt] = (int) Math.abs(conto.tradx[k1] * m.cos(i2) + conto.tradz[k1] * m.sin(i2));
				t.radz[t.nt] = (int) Math.abs(conto.tradx[k1] * m.sin(i2) + conto.tradz[k1] * m.cos(i2));
	...
	public void d(final Graphics2D rd) {
		...
		final int j = m.cz + (int) ((x - m.x - m.cx) * m.sin(m.xz) + (z - m.z - m.cz) * m.cos(m.xz));
		final int k = m.cz + (int) ((y - m.y - m.cy) * m.sin(m.zy) + (j - m.cz) * m.cos(m.zy));
		...
		if (xs(i + maxR * 2, k) > 0 && xs(i - maxR * 2, k) < m.w && k > -maxR && (k < m.fade[disline] + maxR || m.trk)
				...
				if (!m.crs) {
					if (k < 2000) {
						...
						if (flag)
							...
							final int j2 = m.cy + (int) ((m.ground - m.cy) * m.cos(m.zy) - (j - m.cz) * m.sin(m.zy));
							final int k2 = m.cz + (int) ((m.ground - m.cy) * m.sin(m.zy) + (j - m.cz) * m.cos(m.zy));
			...
			final int j1 = m.cy + (int) ((y - m.y - m.cy) * m.cos(m.zy) - (j - m.cz) * m.sin(m.zy));
	...
	public void dust(final int i, final float f, final float f1, final float f2, final float f3, final float f4,
			final float f5, final boolean flag, final int j) {
		boolean flag1 = false;
		if (j > 5 && (i == 0 || i == 2))
			flag1 = true;
		if (j < -5 && (i == 1 || i == 3))
	...
	public void lowshadow(final Graphics2D rd, final int i) {
		...
		for (int k1 = t.nt - 1; k1 >= 0; k1--) {
			...
			do {
				ai1[j2] = t.y[k1] - m.y;
				if (t.zy[k1] != 0)
					ai1[j2] += ((ai2[j2] - (t.z[k1] - m.z - t.radz[k1])) * m.sin(t.zy[k1])) / m.sin(90 - t.zy[k1])
							- (t.radz[k1] * m.sin(t.zy[k1])) / m.sin(90 - t.zy[k1]);
				if (t.xy[k1] != 0)
					ai1[j2] += ((ai[j2] - (t.x[k1] - m.x - t.radx[k1])) * m.sin(t.xy[k1])) / m.sin(90 - t.xy[k1])
							- (t.radx[k1] * m.sin(t.xy[k1])) / m.sin(90 - t.xy[k1]);
			} while (++j2 < 4);
			k = (int) (t.c[k1][0] / 1.5D);
			l = (int) (t.c[k1][1] / 1.5D);
	...
	public void fixit(final Graphics2D rd) {
		if (fcnt == 1)
			for (int i = 0; i < npl; i++) {
		...
		if (fcnt == 1 || fcnt > 2) {
			...
			final int i3 = m.cy + (int) ((y - m.y - m.cy) * m.cos(m.zy) - (l2 - m.cz) * m.sin(m.zy));
			l2 = m.cz + (int) ((y - m.y - m.cy) * m.sin(m.zy) + (l2 - m.cz) * m.cos(m.zy));
			ai[0] = xs((int) (j2 - j1 / 0.80000000000000004D - m.random() * (j1 / 2.3999999999999999D)), l2);
			ai1[0] = ys((int) (i3 - l1 / 1.9199999999999999D - m.random() * (l1 / 5.6699999999999999D)), l2);
			ai[1] = xs((int) (j2 - j1 / 0.80000000000000004D - m.random() * (j1 / 2.3999999999999999D)), l2);
			ai1[1] = ys((int) (i3 + l1 / 1.9199999999999999D + m.random() * (l1 / 5.6699999999999999D)), l2);
			ai[2] = xs((int) (j2 - j1 / 1.9199999999999999D - m.random() * (j1 / 5.6699999999999999D)), l2);
			ai1[2] = ys((int) (i3 + l1 / 0.80000000000000004D + m.random() * (l1 / 2.3999999999999999D)), l2);
			ai[3] = xs((int) (j2 + j1 / 1.9199999999999999D + m.random() * (j1 / 5.6699999999999999D)), l2);
			ai1[3] = ys((int) (i3 + l1 / 0.80000000000000004D + m.random() * (l1 / 2.3999999999999999D)), l2);
			ai[4] = xs((int) (j2 + j1 / 0.80000000000000004D + m.random() * (j1 / 2.3999999999999999D)), l2);
			ai1[4] = ys((int) (i3 + l1 / 1.9199999999999999D + m.random() * (l1 / 5.6699999999999999D)), l2);
			ai[5] = xs((int) (j2 + j1 / 0.80000000000000004D + m.random() * (j1 / 2.3999999999999999D)), l2);
			ai1[5] = ys((int) (i3 - l1 / 1.9199999999999999D - m.random() * (l1 / 5.6699999999999999D)), l2);
			ai[6] = xs((int) (j2 + j1 / 1.9199999999999999D + m.random() * (j1 / 5.6699999999999999D)), l2);
			ai1[6] = ys((int) (i3 - l1 / 0.80000000000000004D - m.random() * (l1 / 2.3999999999999999D)), l2);
			ai[7] = xs((int) (j2 - j1 / 1.9199999999999999D - m.random() * (j1 / 5.6699999999999999D)), l2);
			ai1[7] = ys((int) (i3 - l1 / 0.80000000000000004D - m.random() * (l1 / 2.3999999999999999D)), l2);
			if (fcnt == 3)
			...
			ai1[0] = ys((int) (i3 - l1 / 2.3999999999999999D - m.random() * (l1 / 9.5999999999999996D)), l2);
			ai[1] = xs((int) (j2 - j1 - m.random() * (j1 / 4)), l2);
			ai1[1] = ys((int) (i3 + l1 / 2.3999999999999999D + m.random() * (l1 / 9.5999999999999996D)), l2);
			ai[2] = xs((int) (j2 - j1 / 2.3999999999999999D - m.random() * (j1 / 9.5999999999999996D)), l2);
			ai1[2] = ys((int) (i3 + l1 + m.random() * (l1 / 4)), l2);
			ai[3] = xs((int) (j2 + j1 / 2.3999999999999999D + m.random() * (j1 / 9.5999999999999996D)), l2);
			ai1[3] = ys((int) (i3 + l1 + m.random() * (l1 / 4)), l2);
			ai[4] = xs((int) (j2 + j1 + m.random() * (j1 / 4)), l2);
			ai1[4] = ys((int) (i3 + l1 / 2.3999999999999999D + m.random() * (l1 / 9.5999999999999996D)), l2);
			ai[5] = xs((int) (j2 + j1 + m.random() * (j1 / 4)), l2);
			ai1[5] = ys((int) (i3 - l1 / 2.3999999999999999D - m.random() * (l1 / 9.5999999999999996D)), l2);
			ai[6] = xs((int) (j2 + j1 / 2.3999999999999999D + m.random() * (j1 / 9.5999999999999996D)), l2);
			ai1[6] = ys((int) (i3 - l1 - m.random() * (l1 / 4)), l2);
			ai[7] = xs((int) (j2 - j1 / 2.3999999999999999D - m.random() * (j1 / 9.5999999999999996D)), l2);
	...
	public void electrify(final Graphics2D rd) {
		int i = 0;
		do {
			if (elc[i] == 0) {
			...
			final int j = (int) (edl[i] + (190F - m.random() * 380F));
			final int k = (int) (edr[i] + (190F - m.random() * 380F));
			final int l = (int) (m.random() * 126F);
			final int i1 = (int) (m.random() * 126F);
			final int ai[] = new int[8];
			final int ai1[] = new int[8];
